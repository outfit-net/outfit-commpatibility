...
class DataLoaderPolyvore(Dataloader):
    ...
    def init_phase(self, phase):
        ...
        adj = sp.load_npz(adj_file).astype(np.int32)  # 加载邻接矩阵
        setattr(self, '{}_adj'.format(phase), adj)
        node_features = sp.load_npz(feats_file)  # 加载节点特征
        setattr(self, '{}_features'.format(phase), node_features)

        # get lower tiangle of the adj matrix to avoid duplicate edges
        setattr(self, 'lower_{}_adj'.format(phase), sp.tril(adj).tocsr())  # 只得到下三角的邻接矩阵
        # setattr 设置属性值 getattr得到属性值
    ...
    def get_phase(self, phase):
        ...
        pos_r_idx, pos_c_idx = lower_adj.nonzero()  # 返回lower_adj中不为0的索引值
        pos_labels = np.array(lower_adj[pos_r_idx, pos_c_idx]).squeeze()  # 得到positive边 比如[[1,2][2,3][3,4]]
        ...
        pos_labels, pos_r_idx, pos_c_idx = pos_labels[perm], pos_r_idx[perm], pos_c_idx[perm]  # 将pos edge和索引打乱
        '''
        ...
        before = time.time()
        n_train_neg = eval_pos_labels.shape[0]  # 设置每次迭代需要采样的负训练边数
        ...
        for i in range(n_train_neg):
            ...
            neg_r_idx[i] = r_idx
            neg_c_idx[i] = c_idx
        print('Sampling done, time elapsed: {}'.format(time.time() - before))  # 随机采样生成neg边
        ...
        labels = np.append(eval_pos_labels, neg_labels)  # ground true
...
'''
邻接矩阵adj是所有节点连接边的一般所构成的  另外一般用来验证
'''
