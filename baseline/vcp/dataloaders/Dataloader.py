...
class Dataloader(object):
    ...
    def normalize_features(self, feats, get_moments=False, mean=None, std=None):
        reuse_mean = mean is not None and std is not None   # false
        if feats.shape[1] != 2048: # image features
            raise ValueError('Features are expected to be 2048-dimensional (extracted with ResNet)')
        else:
            feats = np.array(feats.todense())  # 返回一个矩阵
            if reuse_mean:
                mean_feats = mean
                std_feats = std
            else:
                mean_feats = feats.mean(axis=0)  # 取每列的均值，指的是所有节点在每个纬度上的特征值
                std_feats = feats.std(axis=0)  # 求每列的标准差

            # normalize
            feats = (feats - mean_feats)/std_feats  # 归一化
        if get_moments:  # false
            return feats, mean_feats, std_feats
        return feats  # 返回归一化的节点特征
