...
def support_dropout(sup, do, edge_drop=False):
    before = time.time()
    sup = sp.tril(sup)  # 以稀疏格式返回矩阵的下三角部分
    assert do > 0.0 and do < 1.0
    n_nodes = sup.shape[0]
    # nodes that I want to isolate
    isolate = np.random.choice(range(n_nodes), int(n_nodes*do), replace=False)  #从0到n_nodes-1中随机选取n_nodes*do个数字，不重复
    nnz_rows, nnz_cols = sup.nonzero()  # 返回矩阵中不是0的行和列索引值
    ...
    return sup  # 随机dropout输入节点之间的连接关系
...
def normalize_nonsym_adj(adj):  # 归一化邻接矩阵
